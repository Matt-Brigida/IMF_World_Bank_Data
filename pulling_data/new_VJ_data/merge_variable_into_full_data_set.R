...
### Before using imf data check if we need to upcase country in returned data!!!!!
merge_imf <- function(indicator, backup = TRUE){

    ## read in data-----
    olddata <- readRDS("./data_to_VJ.rds")
    ...
    saveRDS(new_data, "data_to_VJ.rds")
...
merge_wb <- function(indicator, backup = TRUE){
    ...
    ind_data$country <- toupper(ind_data$country)
    ...
    ## dont think I need iso2c
    ## new_data <- merge(ind_data, olddata, by = c("country", "year", "iso2c"), all.y = TRUE)
    new_data <- merge(ind_data, olddata, by = c("country", "year"), all.y = TRUE)
